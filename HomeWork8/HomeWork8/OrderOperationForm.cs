...
namespace HomeWork8
{
    public delegate void setOrderValue(string orderId, string clientname,bool vipShip, string deliveryAddress,  OrderDetail orderDetails);
    ...
    public partial class OrderOperationForm : Form
    {
        ...
        private void operateOrderBtn_Click(object sender, EventArgs e)
        {
            ...
            OrderDetail orderDetail = new OrderDetail(goodsID, goodsName, price, amount);

            if (Yes_radioButton.Checked)
                setMainFormOrderValue(orderID, Name, true, address, orderDetail);
            else
                setMainFormOrderValue(orderID, Name, false, address, orderDetail);
            this.Close();
        }
    }
}
